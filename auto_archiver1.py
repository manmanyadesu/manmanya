...
from PIL import Image  # 이미지 압축용 라이브러리
...
START_PAGE = 1
END_PAGE = 2
MAX_POSTS_TO_ARCHIVE = 1  # ⚠️ 테스트를 위해 딱 1개만 수집하도록 세팅했습니다!
...
def compress_image(source_path, target_path, max_width=1000, quality=80):
    """
    만화 원본 이미지를 고밀도로 압축(가로 1000px, 품질 80%)하여 용량을 1/10 수준으로 줄입니다.
    """
    try:
...
def archive_single_post(post_no, page, drive_service, creds):
    ...
    # 3. HTML 로컬 저장 및 [0MB 다이어트] 로컬 임시 이미지 일체 소거
    ...
    # [0MB 완전 실현] 구글 드라이브에 다 올라갔으므로 안심하고 로컬 임시 폴더 영구 삭제!
...
with sync_playwright() as p:
    ...
    def block_heavy_resources(route):
        if route.request.resource_type in ["stylesheet", "font", "media"]: route.abort()
