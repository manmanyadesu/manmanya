...
# ⚡ [초고속 오프라인 로컬 재조립 엔진] 
# 크롬창 로딩 및 구글 드라이브 API 통신 0회! 이미 다운로드된 이미지 주소와 댓글 데이터를 바탕으로 0.01초 만에 템플릿만 복구
def rebuild_html_locally(post_no):
    ...
    try:
        ...
        # 이전 HTML 스크립트 태그 내에 백업된 rawComments JSON 데이터를 정규식으로 직접 추출
        ...
        poll_section_html = f"""<div class="poll-container"><h3>🗳️ 본문 투표 백업</h3><img src="{poll_drive_url}" style="max-width:100%; display:block; margin:0 auto;"></div>""" if (poll_drive_url or has_poll) else ""
        ...
        try:
            for page_num in range(start_p, end_p + 1):
                ...
                for row in soup.select("tr.us-post:not(.notice)"):
                    ...
                    # 🚀 수정한 부분: 로컬 템플릿 초고속 재빌드 호출 적용 완료
