...
FORCE_OVERWRITE = False                 # True로 설정 시 이미 수집된 글도 무조건 원본 이미지를 새로 받습니다.//False
...
# 중복 실행 방지용 락 시스템
# auto_archiver1.py와 같은 락 파일을 사용하여 크롤링뿐 아니라 Git 배포까지 서로 겹치지 않게 합니다.
...
def run_direct_archiver():
    ...
    with sync_playwright() as p:
        ...
        # 원래 자동 배포 방식 유지:
        # Git 내부 자동정리 경고가 나오더라도 커밋이 만들어졌다면 push까지 계속 진행합니다.
        # exFAT에서 .git이 과도하게 커지거나 손상되면 별도로 새 clone으로 교체합니다.
